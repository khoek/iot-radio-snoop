...
static void dev_start(dps368_handle_t dev) {
...
static poll_result_t poll(const char* _tag, dps368_handle_t dev, QueueHandle_t queue) {
    uint8_t meas_cfg = dps368_reg_read(dev, DPS368_REG_MEAS_CFG);
...
static cJSON* report(const char* tag, dps368_handle_t dev, measurement_t* result) {
    ...
    ESP_LOGI(TAG, "(%-10s) temp(oC)=%.7lf, press(Pa)=%.7lf", tag, temp_c, press_pa);
...
sensor_type_t SENSOR_DPS368 = {
    ...
    .dev_reset = (sensor_dev_fn_t) dps368_reset,
    .dev_destroy = (sensor_dev_fn_t) dps368_reset,
